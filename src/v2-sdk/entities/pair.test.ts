...
describe('Pair', () => {
  ...
  describe('constructor', () => {
    it('cannot be used for tokens on different chains', () => {
      expect(
        () => new Pair(CurrencyAmount.fromRawAmount(USDC, '100'), CurrencyAmount.fromRawAmount(WETH9[2370], '100'))
      ).toThrow('CHAIN_IDS')
    })
  })

  describe.skip('#getAddress', () => {
    // This is not the same address anymore, since we changed the Factory contract to our XZO one
  ...
  describe('#priceOf', () => {
    ...
    it('throws if invalid token', () => {
      expect(() => pair.priceOf(WETH9[2370])).toThrow('TOKEN')
    })
  })
