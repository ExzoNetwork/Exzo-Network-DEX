      ...
      {attempting && !hash && (
        <LoadingView onDismiss={wrappedOnDismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.body fontSize={20}>{parsedAmount?.toSignificant(4)} FNBS</TYPE.body>
          </AutoColumn>
        </LoadingView>
      )}
      {attempting && hash && (
        <SubmittedView onDismiss={wrappedOnDismiss} hash={hash}>
          <ConfettiZ
            colors={['#19bcc8', '#25CBE5', '#1dacc2', '#2a8d9c', '#22646ea', '#0C5C92']}
            recycle={false}
            width={1920}
            height={1480}
            numberOfPieces={200}
          ...
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.body fontSize={20}>Staked {parsedAmount?.toSignificant(4)} FNBS</TYPE.body>
...
function AddXDiffButton({ addToken, success }: { addToken: () => void; success?: boolean }) {
  ...
  return <ButtonSecondary onClick={addToken}>Add xFNBS to Metamask</ButtonSecondary>
}
