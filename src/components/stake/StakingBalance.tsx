...
import { EXZOSWAPTOKEN, XEXZOSWAPTOKEN } from 'constants/tokens'
...
export function StakingBalance() {
  ...
  const token = chainId ? EXZOSWAPTOKEN[chainId] : undefined
  const xToken = chainId ? XEXZOSWAPTOKEN[chainId] : undefined
  ...
  const xDiffContractBalance = useTokenBalance(chainId ? XEXZOSWAPTOKEN[chainId].address : undefined, token)
              ...
              <TYPE.largeHeader color={'primary1'} marginBottom={`10px`}>
                <TokenLogo>
                  ...
                  <TYPE.body fontSize={16} fontWeight={500} margin={'10px'}>
                    FNBS / xFNBS Ratio:{' '}
                    <span style={{ color: '#19bcc8', fontSize: '14px', paddingLeft: '7px' }}>
                      {ratio ? (
                        <CountUp
                  ...
                  <QuestionHelper text={`Unstaking 1 xFNBS will earn ${ratio?.toSignificant()} FNBS`} />
                </TokenLogo>
              </TYPE.largeHeader>
            </AutoColumn>
          </RowBetween>
          ...
          <RowBetween>
            ...
            <AutoColumn justify={'end'}>
              <TokenAndUSDCBalance>
                ...
                <span style={{ color: '#19bcc8', fontSize: '14px', paddingLeft: '5px' }}>
                  <span>$</span>
                  {diffusionUSDCValue ? (
                    <CountUp
              ...
              <TokenLogo>
                ...
                <QuestionHelper
                  text={`${earnedDiff?.toFixed(
                    2
                  )} FNBS is available upon unstaking ${xdiffBalance?.toSignificant()} xFNBS.`}
                />
              </TokenLogo>
            </AutoColumn>
